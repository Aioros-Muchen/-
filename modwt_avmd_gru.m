    ...
    if sampEn <= rate
        ...
        for j = 1: imf_size
            ...
            if sampEn <= rate
                ...
                disp(['对第',num2str(i),'个modwt分量的第',num2str(j),'个vmd分量(imf)建模']) % 输出进度。
                ...
                end % predictAndUpdateState函数是一次预测一个值并更新网络 状态
        ...
        disp(['对第',num2str(i),'个modwt分量的vmd趋势分量(imf_trend)建模']) % 输出进度。
        %%  数据处理
