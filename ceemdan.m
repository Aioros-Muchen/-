...
for i=1:NR
    white_noise{i}=randn(size(x));%creates the noise realizations白噪声
end;

for i=1:NR
    modes_white_noise{i}=emd(white_noise{i});%白噪声的imf分量calculates the modes of white gaussian noise
end;

for i=1:NR %calculates the first mode
    temp=x+Nstd*white_noise{i};  %噪声系数取0.2
